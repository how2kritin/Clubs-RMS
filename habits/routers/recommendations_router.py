...
def get_user_profile_with_clubs(user_id: str, db: Session) -> User | None:
    ...
    try:
        ...
        if not user:
            logger.warning(
                f"No user found with uid: {user_id} in get_user_profile_with_clubs"
            )
        return user
    except Exception as e:
        logger.error(
            f"DB error in get_user_profile_with_clubs for user {user_id}: {e}",
            exc_info=True,
        )
        return None
...
async def get_club_recommendations_for_user_strategy(
    ...
    try:
        ...
        logger.error(
            f"Type error during recommendation context creation for user {user_id}: {te}",
            exc_info=True,
        )
        raise HTTPException(
