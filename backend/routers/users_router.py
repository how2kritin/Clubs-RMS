...
from fastapi import (APIRouter, Depends, Response, Request, status, Cookie, HTTPException, Path, Query, Body, )
from sqlalchemy.orm import Session

from models.users.users_config import (get_clubs_by_user, is_admin_of_club, is_member_of_club, user_login_cas,
                                       user_logout, )
...
router = APIRouter(tags=["User Authentication & Profile"], responses={401: {"description": "Not authenticated"}, 403: {
    "description": "Forbidden - insufficient permissions"}, 404: {"description": "User not found"}, 500: {
    "description": "Internal server error"}, }, )


@router.post("/login", status_code=status.HTTP_200_OK, summary="Get CAS Login URL",
...
@router.get("/login", status_code=status.HTTP_200_OK, summary="Process CAS Login",
            ...
            responses={400: {"description": "CAS ticket missing or invalid"},
...
@router.get("/user_info", status_code=status.HTTP_200_OK, summary="Get Current User Info",
            description="Retrieves information about the currently authenticated user.",
            response_description="User information for the authenticated user", response_model=Dict[str, Any], )
...
@router.get("/user_role/{club_id}", status_code=status.HTTP_200_OK, summary="Get User Club Role",
...
async def get_user_role(club_id: str = Path(..., description="The unique identifier of the club"),
...
@router.get("/user_club_info", status_code=status.HTTP_200_OK, response_model=List[ClubOut],
...
async def get_user_club_info(current_user: dict = Depends(get_current_user), db: Session = Depends(get_db), ):
    """
...
@router.get("/is_authenticated", status_code=status.HTTP_200_OK, summary="Check Authentication Status",
...
@router.put("/update_profile", status_code=status.HTTP_200_OK, summary="Update User Profile",
            description="Updates the profile information of the currently authenticated user.",
            response_description="Updated profile information",
            responses={400: {"description": "Invalid profile data"}, 401: {"description": "Not authenticated"},
                       404: {"description": "User not found"}, 500: {"description": "Database error"}})
async def update_user_profile(profile_update: UserProfileUpdate = Body(..., description="User profile data to update",
                                                                       example={
                                                                           "hobbies": "Reading, Photography, Hiking",
                                                                           "skills": ["Python", "React", "SQL"],
                                                                           "profile_picture": 2}),
                              db: Session = Depends(get_db), current_user_data: dict = Depends(get_current_user), ):
    """
    ...
    return {"message": "Profile updated successfully",
            "updated_profile": {"hobbies": db_user.hobbies, "skills": db_user.skills,
