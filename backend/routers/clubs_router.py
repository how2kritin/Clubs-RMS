...
from models.clubs.clubs_config import (fetch_club_by_id, fetch_info_about_all_clubs, is_subscribed, subscribe,
                                       unsubscribe, )
...
router = APIRouter(tags=["Clubs"],
    responses={404: {"description": "Club not found"}, 401: {"description": "Not authenticated"},
        403: {"description": "Forbidden - insufficient permissions"}, }, )


@router.get("/all_clubs", response_model=List[ClubOut], status_code=status.HTTP_200_OK, summary="Get All Clubs",
...
@router.get("/{cid}", response_model=ClubOut, status_code=status.HTTP_200_OK, summary="Get Club by ID",
...
async def get_club_by_id(
        cid: str = Path(..., description="The unique identifier of the club to retrieve", example="cs-club"),
...
@router.get("/is_subscribed/{cid}", response_model=bool, status_code=status.HTTP_200_OK,
    summary="Check Subscription Status", description="Checks if the current user is subscribed to a specific club.",
    response_description="Boolean indicating whether the user is subscribed",
    responses={404: {"description": "Club not found"}})
def get_subscription_status(
        cid: str = Path(..., description="The unique identifier of the club to check subscription status for",
            example="cs-club"), current_user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    """
...
@router.post("/subscribe/{cid}", response_model=ClubOut, status_code=status.HTTP_200_OK, summary="Subscribe to Club",
    ...
    responses={404: {"description": "Club not found"}, 400: {"description": "User is already subscribed to this club"}})
async def create_club_subscription(
        cid: str = Path(..., description="The unique identifier of the club to subscribe to", example="cs-club"),
...
@router.post("/unsubscribe/{cid}", response_model=ClubOut, status_code=status.HTTP_200_OK,
    ...
    responses={404: {"description": "Club not found"}, 400: {"description": "User is not subscribed to this club"}})
async def remove_club_subscription(
        cid: str = Path(..., description="The unique identifier of the club to unsubscribe from", example="cs-club"),
        current_user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    """
