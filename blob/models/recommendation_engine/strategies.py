...
class HobbiesSkillsStrategy(RecommendationStrategy):
    ...
    async def get_recommendations(
        ...
        if user.habits.hobbies:
            ...
            has_content = True
        if user.habits.skills and user.habits.skills != {} and user.habits.skills != []:
...
"""
        logger.info(
            f"Using Hobbies/Skills strategy for user {user.uid}. Generating LLM prompt."
        )
        user_member_cids = self._get_user_member_cids(user)
        recommendations = await self._get_llm_recommendations(
            prompt.strip(), db, all_clubs, user_member_cids
        )
        logger.info(
            f"HobbiesSkillsStrategy for user {user.uid} returning {len(recommendations)} recommendations."
        )
...
class CurrentClubsStrategy(RecommendationStrategy):
    ...
    async def get_recommendations(
        ...
        for c in current_clubs:
            current_club_details.append(
                f"- {c.name} (ID: {c.cid}, Description: {c.description[:100]}...)"
            )  # Add some description context
...
"""
        logger.info(
            f"Using Current Clubs strategy for user {user.uid}. Generating LLM prompt."
        )
        user_member_cids = self._get_user_member_cids(user)
        recommendations = await self._get_llm_recommendations(
            prompt.strip(), db, all_clubs, user_member_cids
        ...
        logger.info(
            f"CurrentClubsStrategy for user {user.uid} returning {len(recommendations)} recommendations."
        )
        return recommendations
