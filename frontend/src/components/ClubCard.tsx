...
const ClubCard: React.FC<ClubCardProps> = ({ logo, name, tagline }) => {
    // Basic error handling or placeholder for missing logo
    const logoSrc = logo ? `http://clubs.iiit.ac.in/files/download?filename=${encodeURIComponent(logo)}` : placeholder;
    console.log(logoSrc);
    ...
    return (
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md overflow-hidden transform transition duration-300 hover:scale-105">
            <div className="p-4 text-center">
                ...
                />
                {/* Club Name */}
                <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-1 truncate" title={name}>
                    {name}
                </h3>
                ...
                {/* Example: <Link to={`/clubs/${cid}`}>View Details</Link> */}
            </div>
        </div>
    );
};
